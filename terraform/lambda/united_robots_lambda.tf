resource "aws_lambda_function" "wire_united_robots" {
  ...
  environment {
    variables = {
      ARC_API_URL       = local.ssm_united_robots.ARC_API_URL
      ARC_API_KEY       = local.ssm_united_robots.ARC_API_KEY
      CLIENT_SECRET_KEY = local.ssm_united_robots.CLIENT_SECRET_KEY
      SECRET_KEY        = local.ssm_united_robots.SECRET_KEY
    }
  }
}
...
resource "aws_lambda_function" "wire_united_robots_rss_feed" {
  ...
  environment {
    variables = {
      ARC_API_URL          = local.ssm_united_robots.ARC_API_URL
      ARC_API_KEY          = local.ssm_united_robots.ARC_API_KEY
      RSS_SINGLE_SALES_URL = local.ssm_united_robots.RSS_SINGLE_SALES_URL
      RSS_SUMMARIES_URL    = local.ssm_united_robots.RSS_SUMMARIES_URL
      CLIENT_SECRET_KEY    = local.ssm_united_robots.CLIENT_SECRET_KEY
      SECRET_KEY           = local.ssm_united_robots.SECRET_KEY
    }
  }
