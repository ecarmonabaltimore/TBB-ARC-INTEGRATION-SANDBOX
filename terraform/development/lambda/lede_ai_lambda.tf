resource "aws_lambda_function" "wire_ledeai" {
  ...
  environment {
    variables = {
      ARC_API_URL       = local.ssm_ledeai_dev.ARC_API_URL
      ARC_API_KEY       = local.ssm_ledeai_dev.ARC_API_KEY
      CLIENT_SECRET_KEY = local.ssm_ledeai_dev.CLIENT_SECRET_KEY
      SECRET_KEY        = local.ssm_ledeai_dev.SECRET_KEY
    }
  }
}
...
resource "aws_lambda_function" "wire_ledeai_rss_feed" {
  ...
  environment {
    variables = {
      ARC_API_URL       = local.ssm_ledeai_dev.ARC_API_URL
      ARC_API_KEY       = local.ssm_ledeai_dev.ARC_API_KEY
      RSS_API_URL       = local.ssm_ledeai_dev.RSS_API_URL
      CLIENT_SECRET_KEY = local.ssm_ledeai_dev.CLIENT_SECRET_KEY
      SECRET_KEY        = local.ssm_ledeai_dev.SECRET_KEY
    }
  }
