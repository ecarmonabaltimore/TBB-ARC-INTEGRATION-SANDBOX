data "aws_ssm_parameters_by_path" "ledeai_prod_ssm_parameters" {
...
locals {
  ssm_ledeai_prod = zipmap([for key in data.aws_ssm_parameters_by_path.ledeai_prod_ssm_parameters.names : replace(key, "/${var.functions.wire_ledeai.ssm_prefix}/${var.environment}//", "")], data.aws_ssm_parameters_by_path.ledeai_prod_ssm_parameters.values)
}
