...
export class WireHandler {
  ...
  public async handleFetchRSSEvent(): Promise<HandlerPutResponse> {
    ...
    const feedItems = await getRSSItemsbyUUID();
    ...
    try {
      ...
      for (let indexItem = 0; indexItem < feedItems.items.length; indexItem++) {
        ...
        storiesID.push(await createStory(feedItems.items[indexItem]));
      }
